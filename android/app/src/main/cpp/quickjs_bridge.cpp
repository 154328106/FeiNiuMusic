// FeiNiuMusic — QuickJS JNI 桥。
//
// 移植自 Lyrico (https://github.com/Replica0110/Lyrico)，Apache-2.0。
// 提供 Java/Kotlin 与 QuickJS (quickjs-ng 0.14.0) 之间的交互接口：
// createRuntime / eval / call / closeRuntime，以及 __lyricoHostCall
// （插件 globalThis.Platform.* 同步调用宿主能力的桥）。
// JNI 类名从 com.lonx.lyrico.plugin.runtime.QuickJsNative 改为
// com.feiniu.music.match.QuickJsNative。
...
Java_com_feiniu_music_match_QuickJsNative_createRuntime(
        JNIEnv *env,
        jobject,
...
Java_com_feiniu_music_match_QuickJsNative_eval(
        JNIEnv *env,
        jobject,
        jlong runtimePtr,
        jstring script,
...
extern "C" JNIEXPORT jstring JNICALL
Java_com_feiniu_music_match_QuickJsNative_call(
        JNIEnv *env,
        jobject,
        jlong runtimePtr,
...
Java_com_feiniu_music_match_QuickJsNative_closeRuntime(
        JNIEnv *,
        jobject,
        jlong runtimePtr) {
    if (runtimePtr == 0) {
