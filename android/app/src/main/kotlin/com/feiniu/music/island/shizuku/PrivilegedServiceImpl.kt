...
/**
 * Shizuku 特权服务实现（跑在 Shizuku 子进程 [ShizukuServiceConnection] 中）。
 *
 * 通过反射直接调用 ConnectivityService 的隐藏方法 setFirewallChainEnabled /
 * setUidFirewallRule，以高权限跨进程方式拦截/恢复 XMSF 网络，从而绕过澎湃 OS
 * 对焦点通知应用的白名单限制。完整移植自 HyperNotification 的
 * PrivilegedServiceImpl（Apache-2.0）。
 */
@Keep
