...
飞牛私有云（FNOS）平台的第三方音乐客户端。通过飞牛 NAS 自带的音乐服务 API（`/music/api/v1/`）获取音乐库、播放流和歌词数据，提供完整的在线音乐播放体验。
...
- **FN Connect 连接** — 支持 FNID 自动探测连接，内网/公网 IPv4/IPv6/中继多层链路探测，支持公网优先和中继优先两种连接偏好
...
- 集成 GitHub Actions CI 自动化构建签名 APK
- 净化和精简上游冗余代码，适配飞牛场景
